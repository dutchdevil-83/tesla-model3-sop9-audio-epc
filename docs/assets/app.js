...
function setupPan(){const v=$('#viewport');v.addEventListener('pointerdown',e=>{if(e.target.closest('.zoom')||e.target.closest('.schematic-message'))return;dragging=true;dragStart={x:e.clientX-panX,y:e.clientY-panY};v.setPointerCapture(e.pointerId);});v.addEventListener('pointermove',e=>{if(!dragging)return;panX=e.clientX-dragStart.x;panY=e.clientY-dragStart.y;applyTransform();});v.addEventListener('pointerup',()=>dragging=false);v.addEventListener('pointercancel',()=>dragging=false);v.addEventListener('wheel',e=>{if(!e.ctrlKey)return;e.preventDefault();zoom(e.deltaY<0?.12:-.12);},{passive:false});}
